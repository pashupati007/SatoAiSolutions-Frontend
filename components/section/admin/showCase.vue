<template>
  <div>
    <v-container>
      <v-data-table :headers="headers" :items="showCases" :loading="loading">
        <template v-slot:top>
          <v-toolbar flat color="primary">
            <v-toolbar-title>Show Case</v-toolbar-title>
            <v-spacer></v-spacer>
        ...
        <template v-slot:no-data>
          <v-btn color="primary" @click="getShowCases"> Reset </v-btn>
        </template>
      </v-data-table>
    </v-container>
  </div>
...
export default {
  ...
  methods: {
    ...
    editItem(item) {
      this.editedIndex = this.showCases.indexOf(item);
    ...
    deleteItemConfirm() {
      try {
        this.$axios
          .delete("/showCase/" + this.editedItem.id)
          .then((response) => {
            this.showCases.splice(this.editedIndex, 1);
            this.closeDelete();
          });
      } catch (error) {
        console.error("Error fetching data:", error);
